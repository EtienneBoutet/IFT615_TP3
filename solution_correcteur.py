...
class Correcteur:
    ...
    def corrige(self, mot):
        ...
        alpha = np.empty((K, T), dtype=object)
        
        for i in range(K):
            prob = self.p_observation[self.letters2int[mot[0]], i] * self.p_init[i]
            alpha[i][0] = (prob, None)

        # alpha[:, 0] = (self.p_init * self.p_observation[self.letters2int[mot[0]], :], None)
        ...
        for t in range(1, T):
            for i in range(K):
                ...
                provientDe = -1
                for j in range(K):
                    candidatMax = self.p_transition[i, j] * alpha[j, t-1][0]
                    if candidatMax > max:
                        max = candidatMax
                        provientDe = j

                prob = self.p_observation[self.letters2int[mot[t]], i] * max
                alpha[i][t] = (prob, provientDe)

                # alpha[i, t] = self.p_observation[self.letters2int[mot[t]], i] * np.max(alpha[:, t - 1] * self.p_transition[:, i])
